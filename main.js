...
var prototypefabric = new function () {
	this.initCanvas = function () {
        ...
        document.getElementById('loadPolygonCanvas').onclick = function () {
            const cpolygone = document.createElement('canvas');
            cpolygone.setAttribute('id', 'cpolygone');
            document.getElementById('c').parentElement.append(cpolygone);
            var hammertime = new Hammer(cpolygone);
            hammertime.on('tap', function(ev) {
                console.log(ev);

                if (polygonMode || !pointArray.length) {
                    prototypefabric.polygon.addPoint({e: {layerX: ev.center.x, layerY: ev.center.y}});
                }
                if (ev.target && pointArray.length && ev.target.id == pointArray[0].izd) {
                    prototypefabric.polygon.generatePolygon({e: {layerX: ev.center.x, layerY: ev.center.y}});
                }

                let eventMove = new CustomEvent("mouse:move", {e: {layerX: ev.center.x, layerY: ev.center.y}});
                let eventDown = new CustomEvent("mouse:down", {e: {layerX: ev.center.x, layerY: ev.center.y}});

                document.getElementById('c').dispatchEvent(eventMove);
                document.getElementById('c').dispatchEvent(eventDown);
		...
		canvas.on('mouse:down', function (options) {
			...
			if (options.target && options.target.id == pointArray[0].izd) {
